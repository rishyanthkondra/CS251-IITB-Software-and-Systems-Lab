...
public interface DaoAccess {
    ...
    @Query("SELECT * FROM " + MyDatabase.TABLE_NAME_TODO + " WHERE (category = null OR category = '') AND (name = :Name)")
    Todo fetchAllTodos(String Name);

    @Query("SELECT * FROM "+ MyDatabase.TABLE_NAME_TODO + " WHERE name = :Name")
    List<Todo> fetchSpecificTodos(String Name);

    @Query("SELECT * FROM " + MyDatabase.TABLE_NAME_TODO + " WHERE categoryp = :category")
